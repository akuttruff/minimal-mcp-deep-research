...
**`instant_answer`** — Direct factual answers from DuckDuckGo's Instant Answer API, sourced from Wikipedia and other knowledge bases. Best for definitions, people, places, and "what is X" queries. Returns nothing for obscure topics — fall back to `research` in that case.
